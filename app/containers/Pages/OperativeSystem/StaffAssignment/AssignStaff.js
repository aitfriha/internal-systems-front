...
import { getEligibleStaff } from '../../../../redux/staffAssignment/actions';
...
export class AssignStaff extends React.Component {
  ...
  onDragEnd(destination, source) {
    let { eligibleStaff, assignedStaff } = self.props.data;

    for (const key in eligibleStaff) {
      var staffFind ;
      if(assignedStaff !== undefined && assignedStaff !== null && assignedStaff.length > 0)
    {staffFind = eligibleStaff.filter(staff => staff.staffId === assignedStaff[key].id);}
  
   
      if (staffFind) {
        eligibleStaff = eligibleStaff.splice(staffFind);
  ...
  handleSave() {
    const {
      updateOperationAssignment, getStaffAssignedByOperation, handleClose, data, getEligibleStaff
    } = self.props;
    ...
    promise.then((result) => {
      ...
      if (isString(result)) {
        ...
        getEligibleStaff(data.operation.commercialOperationId);
        self.state.sendList = [];
...
Assign staff to selected operation
        ...
        <DialogActions style={{ minWidth: 585, maxWidth: 585 }}>
          ...
          <Button variant="contained" size="small" disabled={this.state.sendList.length == 0 }  onClick={this.handleSave} color="primary">
            Save
