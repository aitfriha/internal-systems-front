...
import ShowSelectionProcess from './showSelectionProcess';
...
class SelectionProcessInformation extends React.Component {
  constructor(props) {
    ...
    this.state = {
      economicCompanyProposalInEuroProfile: '',
      isShowCandidateDialog: false,
      comment: '',
      numPages: null,
      pageNumber: null,
            ...
            customBodyRender: (value, tableMeta) => (
              <React.Fragment>
                <Tooltip title="View candidate">
                  <IconButton
                    onClick={() => this.viewCandidateProfile(tableMeta)}
                  >
  ...
  componentDidMount() {
    ...
    promise.then(result => {
      ...
      //  economicCompanyProposalInEuroProfile: newProcess.economicCompanyProposalInEuro
      });
    });
  }
  ...
  viewCandidateProfile = (tableMeta) => {
    ...
    const economicClaimsValueInEuro = parseFloat(selectionProcessInformationSelected.economicClaimsValue) * factor;
    let economicClaimsRange1 = '***';
    let economicClaimsRange2 = '***';
    if (selectionProcessInformationSelected.economicClaimsType === 'Number') {
    //  economicClaimsRange1InEuro = '-';
    //  economicClaimsRange2InEuro = '-';
    } else {
      economicClaimsRange1 = parseFloat(selectionProcessInformationSelected.economicClaimsRange1) * factor;
      economicClaimsRange2 = parseFloat(selectionProcessInformationSelected.economicClaimsRange2) * factor;
    }
    this.setState({
      isShowCandidateDialog: true,
      selectionProcess: selectionProcessInformationSelected,
  ...
  handleBack = () => {
    this.setState({ isShowCandidateDialog: false });
  };
  ...
  handleOpenEditDialog = tableMeta => {
    ...
    this.setState({
      selectionProcessInformationSelected,
      comment: selectionProcessInformationSelected.comment,
      subjectiveMark: selectionProcessInformationSelected.subjectiveMark,
      english: selectionProcessInformationSelected.english,
  ...
  render() {
    ...
    const excludeAttributes = ['selectionProcessId', 'economicProposalDoc', 'curriculumDoc', 'curriculumDoc', 'attitudeTestDoc', 'currencyId'];
    ...
    const options = {
      ...
      customToolbar: () => (
        <CustomToolbar
          ...
          fileName="selection process information"
      ...
      <div>
        ...
        >
          ...
          <DialogContent>
            ...
            {selectionProcessInformationSelected != null && selectionProcessInformationSelected.knowledge.length > 0 ? (<div />) : (<div>there is no experiences for this personne</div>)}
          </DialogContent>
          <DialogActions>
            <Button autoFocus onClick={this.handleClose} color="primary">
              Close
        ...
        >
          <Slide
            direction="right"
            in={!isShowCandidateDialog}
            style={{ transitionDelay: isShowCandidateDialog ? '500ms' : '0ms' }}
            mountOnEnter
            unmountOnExit
          >
            <div>
          ...
          >
            <div>
              ...
              <ShowSelectionProcess
                ...
                economicCandidateProposalInEuro={economicCandidateProposalInEuro}
                objectivesInEuro={objectivesInEuro}
                economicCompanyProposalInEuro={economicCompanyProposalInEuro}
                economicClaimsValueInEuro={economicClaimsValueInEuro}
                economicClaimsRange1={economicClaimsRange1}
                economicClaimsRange2={economicClaimsRange2}
              />
              <IconButton onClick={() => this.handleBack()}>
                <KeyboardBackspaceIcon color="secondary" />
              </IconButton>
