...
import {
  addSectorCompany, deleteConfirmationSectorCompany, deleteSectorCompany, updateSectorCompany,
...
class sector extends React.Component {
  ...
  edit = (a, b, c) => {
    ...
    promise.then((result) => {
      ...
      const sector3 = allSectorComapnys.filter(row => (row.secondSectorName === b && row.thirdSectorName === c))[0];
      this.setState({ thirdSectorName: sector3.thirdSectorName });
      this.setState({ description3: sector3.thirdSectorDescription });
      if (sector3.thirdSectorId == undefined) {
        this.setState({ thirdSectorId: '' });
      } else { this.setState({ thirdSectorId: sector3.thirdSectorId }); }
    });
  ...
  handleUpdateSector= () => {
    const { updateSectorCompany, getAllSectorCompany, getAllPrimarySectorCompany } = this.props;
    const {
  ...
  render() {
    ...
    const {
      description1, description2, description3, thirdSectorName, isDisabled, openPopUp, message, keySectorPrimary, keySectorSecond, openDialog
    } = this.state;
    const {
      // eslint-disable-next-line no-shadow
    ...
    const excludeAttributes = ['firstSectorId', 'secondSectorId', 'thirdSectorId'];
    ...
    const options = {
      ...
      print: exportButton,
      downloadOptions: { filename: 'Sectors.csv' },
      rowsPerPage: 10,
      customToolbar: () => (
        <CustomToolbar
          excludeAttributes={excludeAttributes}
          csvData={allSectorComapnys}
          url="/app/gestion-commercial/sectorsCompany/addSector"
        ...
        <PapperBlock title="Sectors" icon="ios-person" noMargin>
          {thelogedUser.userRoles[0].actionsNames.commercial_sectorsCompany_create ? (
            <Dialog
              ...
              maxWidth="md"
            >
              <DialogTitle id="alert-dialog-slide-title"> update Sector </DialogTitle>
