    ...
    handleSave = () => {
      ...
      ActionTypeService.updateActionType(ActionType).then(result => {
        if (result.status === 200) {
          notification('success', 'Action type updated');
          this.setState({ datas: result.data.payload });
        }
      })
        .catch(err => notification('danger', err.response.data.errors));
    ...
    const options = {
      ...
      customToolbar: () => (
        <CustomToolbar
          ...
          csvData={datas}
          fileName="Commercial Action Types"
    ...
    return (
      <div>
        ...
        <Dialog
          ...
          maxWidth=""
        >
          <DialogTitle id="alert-dialog-slide-title" onClick={this.deleteAndUpdateServiceType}> Delete</DialogTitle>
          <DialogContent dividers>
