...
class SuppliersTypeBlock extends React.Component {
  ...
  componentDidMount() {
    ...
    handleDelete = (tableMeta) => {
      ...
      this.setState({ supplierTypeIdDelete: id });
    };
    deleteConfirmeSupplierType= () => {
      const { supplierTypeIdDelete } = this.state;
      this.setState({ openPopUpDelete: false });
      // notification('danger', 'this company is used in other modules !');
      SuppliersTypeService.deleteSuppliersType(supplierTypeIdDelete ).then(result => {
        if (result.status === 200) {
          notification('success', 'Supplier type deleted');
        } else {
          notification('danger', 'Supplier type not deleted');
        }
        this.setState({ datas: result.data });
      });
    };
    ...
    render() {
      ...
      const excludeAttributes = ['supplierTypeId']
      const options = {
        filter: true,
      ...
      return (
        <div>
          ...
          <Dialog
                  ...
                  maxWidth="md"
                >
                  <DialogTitle id="alert-dialog-slide-title"> Delete supplier type</DialogTitle>
                  <DialogContent dividers>
                  ...
                  <DialogActions>
                    ...
                    <Button
                      variant="contained"
                      color="primary"
                      onClick={this.deleteConfirmeSupplierType}
                    >
                        Delete
                    </Button>
                  </DialogActions>
