...
const inputSupplierContractDoc = React.createRef();
...
class AddSuppliersContract extends React.Component {
  ...
  readURI(e) {
    ...
    handleSubmit = () => {
      const {
        name, codeContract, codeSupplier, document, type, typeClient, supplierContractDocExtension, supplierContractDoc,
      ...
      if (supplierContractDoc.constructor === File) {
        formData.append('supplierContractDoc', supplierContractDoc);
      } else {
        formData.append(
          'supplierContractDoc',
          new Blob([], {
      ...
      Object.keys(SuppliersContract).forEach(e => formData.append(e, SuppliersContract[e])
      );
      SuppliersContractService.saveSuppliersContract(formData).then(result => {
  ...
  handleUploadDocumentClick = () => {
    inputSupplierContractDoc.current.value = '';
    inputSupplierContractDoc.current.click();
  };

  handleSupplierContractDocChange = () => {
    const lastDot = inputSupplierContractDoc.current.files[0].name.lastIndexOf('.');
    const ext = inputSupplierContractDoc.current.files[0].name
      .substring(lastDot + 1)
      .toLowerCase();
    if (extList.includes(ext)) {
      this.setState({
        supplierContractDoc: inputSupplierContractDoc.current.files[0],
        supplierContractDocExtension: ext
      });
    }
  };
  ...
  render() {
    ...
    const {
      ...
      contractClient, poClient, typeClient, clients, clientId, contractsClient, contractId, purchaseOrdersClient, purchaseOrderId, supplierContractDoc
    } = this.state;
    return (
      <div>
        <Helmet>
            ...
            <Grid item xs={12} md={6}>
              ...
              <Grid item xs={12} md={6}>
                <Button
                  ...
                  startIcon={<Image color="primary" />}
                  className={
                    supplierContractDoc.constructor === Object
                ...
                <input
                  ...
                  ref={inputSupplierContractDoc}
                  multiple={false}
                  style={{ display: 'none' }}
                  onChange={this.handleSupplierContractDocChange}
                />
                <br />
                <br />
                {
