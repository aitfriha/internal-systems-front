  ...
  componentDidMount() {
    ...
    handleDelete = (tableMeta) => {
      ...
      if (test) this.setState({ openWarning: true });
        ...
        this.setState({
          typeOfCurrencyToDeleteId:id,
          openPopUpDelete:true
        })
      }
    };
    deleteConfirme= () => {
      const { typeOfCurrencyToDeleteId } = this.state;
      this.setState({ openPopUpDelete: false });
              TypeOfCurrencylService.deleteTypeOfCurrency(typeOfCurrencyToDeleteId).then(result => {
                if (result.status === 200) {
                  notification('success', 'Currency type deleted');
                } else {
                  notification('danger', 'Currency Type not deleted');
                }
          this.setState({ datas: result.data });
        });
    ...
    render() {
      ...
      const excludeAttributes = ['typeOfCurrencyId']
      const options = {
        filter: true,
      ...
      return (
        <div>
          ...
          <Dialog
                  ...
                  maxWidth="md"
                >
                  <DialogTitle id="alert-dialog-slide-title"> Delete Currency type</DialogTitle>
                  <DialogContent dividers>
