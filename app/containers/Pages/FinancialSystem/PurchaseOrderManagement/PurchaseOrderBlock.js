...
class PurchaseOrderBlock extends React.Component {
  ...
  handleDelete = (tableMeta) => {
    ...
    this.setState({ purchaseOrderToDeleteId: datas[index].purchaseOrderId });
  };

  deleteConfirmeContract= () => {
    const { purchaseOrderToDeleteId } = this.state;

    PurchaseOrderService.deletePurchaseOrder(purchaseOrderToDeleteId).then(result1 => {
  ...
  handleSave = () => {
    ...
    const PurchaseOrder = {
      ...
      receptionSupplierData: receptionSupplierExternal+ '' + receptionSupplierInternal,
      contractId
    };
    ...
    render() {
      ...
      const excludeAttributes = ['purchaseOrderId','internLogo','companyLogo','termDescription','termTitle','termsListe','nbrConcepts','itemNames','description','unityValue','unity','unityNumber','valor','paymentDate','givingDate','billingDate','companyEmit','externalSupplierReception','iva','client','currency','financialContract'];
      ...
      return (
        <div>
          ...
          <Dialog
            ...
            maxWidth=""
          >
            <DialogTitle id="alert-dialog-slide-title"> Delete purchase order</DialogTitle>
            <DialogContent dividers>
