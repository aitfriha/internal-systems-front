...
class EconomicStaffBlock extends React.Component {
  constructor(props) {
    ...
    this.state = {
      economicStaffRelations: {},
      economicStaffToDeleteId: '',
        ...
        },
        {
          ...
          options: {
            ...
            customBodyRender: (value) => (
              <React.Fragment>
                {
                  (value === '' || value === undefined) ? '---' : moment(value).format('DD/MM/YYYY')
                }
              </React.Fragment>
            )
          }
    ...
    handleDelete = (tableMeta) => {
      const index = tableMeta.tableState.page * tableMeta.tableState.rowsPerPage
            + tableMeta.rowIndex;
      const  economicStaffToDeleteId  = this.state.datas[index].economicStaffId;
      this.setState({ economicStaffToDeleteId: this.state.datas[index].economicStaffId });

      EconomicStaffService.checkIfEconomicStaffFree(economicStaffToDeleteId).then(result => {
        console.log(result.data);
      this.setState({...this.state, economicStaffRelations: result.data});
      this.setState({ openPopUpDelete: true });
    });
    };

  deleteConfirmeStaffEconomic = () => {
    const { economicStaffToDeleteId } = this.state;
    EconomicStaffService.deleteEconomicStaff(economicStaffToDeleteId).then(result => {
  ...
  handleCloseDelete = () => {
    ...
    render() {
      ...
      const excludeAttributes = ['economicStaffId','staff','financialCompany','currency'];
      const options = {
        filter: true,
      ...
      return (
        <div>
                ...
                >
                  <Grid item xs={12} md={3}>
                    ...
                    <KeyboardDatePicker
                      ...
                      value={yearPayment}
                      onChange={value => this.handleDateValue(value, 'yearPayment')
                      }
                      KeyboardButtonProps={{
                ...
                >
                  <Grid item xs={12} md={3}>
                    ...
                    <KeyboardDatePicker
                      ...
                      value={monthPayment}
                      onChange={value => this.handleDateValue(value, 'monthPayment')
                      }
                      KeyboardButtonProps={{
                        'aria-label': 'change date'
                      }}
                ...
                >
                  <Grid item xs={12} md={3}>
                    ...
                    <KeyboardDatePicker
                      ...
                      onChange={value => this.handleDateValue(value, 'extraordinaryDate')
                      }
                      KeyboardButtonProps={{
                        'aria-label': 'change date'
                      }}
          ...
          <Dialog
            ...
            maxWidth=""
          >
            <DialogTitle id="alert-dialog-slide-title"> Delete economic staff</DialogTitle>
            <DialogContent dividers>
              Are you sure you want to delete this economic staff ?
            </DialogContent>
            <DialogContent dividers>
            {this.state.economicStaffRelations.month!== undefined ?
              <li style={{color:blue[700]}}>
              {this.state.economicStaffRelations.month}
            </li>:null}
            {this.state.economicStaffRelations.year!== undefined ?
              <li style={{color:yellow[700]}}>
              {this.state.economicStaffRelations.year}
            </li>:null}
            {this.state.economicStaffRelations.extra!== undefined ?
              <li style={{color:green[700]}}>
              {this.state.economicStaffRelations.extra}
            </li>:null}
            ...
            <DialogActions>
              ...
              <Button
                variant="contained"
                color="primary"
                onClick={this.deleteConfirmeStaffEconomic}
              >
                Delete
              </Button>
            </DialogActions>
